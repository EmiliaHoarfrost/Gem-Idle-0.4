...
@onready var texture: TextureRect = $"../ButtonLuck/TextureRect"
@onready var display: Label = $LuckDisplay
...
func _process(delta: float) -> void:
	PlayerVariables.points += ease(randf(), 10) * PlayerVariables.luck * delta * (PlayerVariables.points / 1000)
#----------------
...
func _button_pressed() -> void:
	if PlayerVariables.points >= gain_cost:
		...
		PlayerVariables.luck += 1
		gain_cost = gain_cost * cost_factor
		if OS.is_debug_build():
			print("Your luck level is at: " + str(round(PlayerVariables.luck)) +".")
		console.text = "Your luck level is at: " + str(round(PlayerVariables.luck)) +"."
		display.text = "x" + (str(format_large_number(PlayerVariables.luck)))
		apply_shake()
		play_sound_effect()
