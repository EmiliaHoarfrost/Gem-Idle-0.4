...
@onready var texture: TextureRect = $"../ButtonGain/TextureRect" 
@onready var display: Label = $GainDisplay
#----------------
# SHAKE VARIABLES
...
func _button_pressed() -> void:
	if PlayerVariables.points >= gain_cost:  # Ensure you have enough points
		PlayerVariables.points -= gain_cost  # Deduct points
		PlayerVariables.points_per_click += 1  # Upgrade points per click
		gain_cost = gain_cost * cost_factor
		if OS.is_debug_build():
			print("You bought an upgrade: " + str(PlayerVariables.points_per_click) +".")
		console.text = "You bought an upgrade: " + str(PlayerVariables.points_per_click) + "."
		display.text = "x" + (str(format_large_number(PlayerVariables.points_per_click)))
